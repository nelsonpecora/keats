...
khi.factory 'FeedService', ($http) ->
    new class FeedService
        posts: null
        get: ->
            feed = $http.jsonp 'https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&num=6&callback=JSON_CALLBACK&q=http://blog.keats.me/rss'
            feed.then (res) =>
...
khi.controller 'HomePageCtrl', (Projects, FeedService) ->
    new class HomePage
        constructor: ->
            FeedService.get().then () => @blogposts = FeedService.posts
        projects: Projects

khi.controller 'WorkPageCtrl', (Projects) ->
    new class WorkPage
        projects: Projects
